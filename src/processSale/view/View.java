...
public class View {
    ...
    public View(Controller contr) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            infoToBeDisplayed = registerItem("Banana", 3);
            printToDisplay();

            // HARDCODED: "Apple" triggers artificial database failure.
            infoToBeDisplayed = registerItem("Apple", 1);
            printToDisplay();

            infoToBeDisplayed = registerItem("Kiwi", 5);
            printToDisplay();

            infoToBeDisplayed = registerItem("Helicopter propeller", 15);
            printToDisplay();

            infoToBeDisplayed = registerItem("Banana", 3);
            printToDisplay();

            infoToBeDisplayed = registerItem("Strawberries", 2);
            printToDisplay();

            infoToBeDisplayed = registerItem("Helicopter gyroscope", 2);
            printToDisplay();

            infoToBeDisplayed = registerItem("Kiwi", 2);
            printToDisplay();

            infoToBeDisplayed = registerItem("Banana", -1);
            printToDisplay();
